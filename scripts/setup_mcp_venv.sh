...
"$VENV_DIR/bin/pip" install -r "$PROJECT_ROOT/mcp_server/requirements.txt"

echo "Done. Add this to your MCP client config (command/args):"
echo "  \"command\": \"$VENV_DIR/bin/python\","
echo "  \"args\": [\"$PROJECT_ROOT/mcp_server/server.py\"],"
echo "  \"cwd\": \"$PROJECT_ROOT\","
echo "  \"env\": { \"PYTHONPATH\": \"$PROJECT_ROOT:$PROJECT_ROOT/backend\" }"
echo ""
echo "See mcp_server/README.md for full setup notes."

...
"$VENV_DIR/bin/pip" install -r "$PROJECT_ROOT/mcp_requirements.txt"

echo "Done. Add this to your Claude MCP config (command/args):"
echo "  \"command\": \"$VENV_DIR/bin/python\","
echo "  \"args\": [\"$PROJECT_ROOT/mcp_server.py\"],"
echo "  \"cwd\": \"$PROJECT_ROOT\","
echo "  \"env\": { \"PYTHONPATH\": \"$PROJECT_ROOT\" }"
echo ""
echo "Or run: cat $PROJECT_ROOT/claude_mcp_config.json"
